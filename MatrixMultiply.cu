 ...
 */
__constant__ int b=16;  // Block Size
__global__ void 
MatrixMul(float* A, float* B, float* C)
{
	...
	for(int k=0; k<WIDTH; k+=b)
    {

	sA[rowi][coli] = A[row*WIDTH + col];
	sB[rowi][coli] = B[row*WIDTH + col];
	__syncthreads();
	
	        for(int kk=0; kk<b; kk++)
            {
			  sC[rowi][coli] += sA[rowi][kk]*sB[kk][coli];
		    }
		    C[row*WIDTH+col] = sC[rowi][coli];
	}
}
